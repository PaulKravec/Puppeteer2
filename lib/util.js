...
module.exports = {
  ...
  getFreeRandomChair: async function (page) { // выбор свободного случайного кресла
  ...
  selectDate: async function (page, day) { // выбор дня недели вперед от текущей даты
    await page.waitForSelector("h1");
    let daysOfWeek = await page.$$("a.page-nav__day"); // выбрать дату - послезавтрашнее число
    await daysOfWeek[day].click();
  },

  selectHall: async function (page, index1) { // выбор зала 
